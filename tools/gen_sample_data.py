...
def generate(
    n_clients: int, months: int, seed: int, outdir: str, as_of: datetime
) -> tuple[str, str]:
    ...
    for i in range(1, n_clients + 1):
        ...
        if profile == "loyal_active":
            ...
            interval, n_visits, stop_days_ago = (
                rnd.choice([7, 9]),
                rnd.randint(14, 30),
                rnd.randint(60, 150),
            )
        ...
        usual_hour = rnd.choice(
            [9, 10, 11, 12, 19, 20, 21] if profile != "loyal_active" else [19, 20, 21]
        )
    ...
    for a, b in zip(pair_pool[::2], pair_pool[1::2]):
        a_books = [
            x for x in bookings if x["client_id"] == f"CL{a:04d}" and x["status"] == "состоялась"
        ]
...
def simulate_future(
    bookings_path: str, out_path: str, returned_ids: list[str], as_of: datetime, seed: int = 7
) -> str:
    ...
    for cid in returned_ids:
        ...
        for k in range(n):
            ...
            dt = dt.replace(
                hour=rnd.choice([10, 11, 12, 19, 20]), minute=0, second=0, microsecond=0
            )
            rows.append(
                {
                    "booking_id": f"B{bid:06d}",
